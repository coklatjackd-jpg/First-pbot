...
const ALLOWED_COMMAND_GROUPS = ['Built-In', 'Group Management', 'File Tools', 'Downloader', 'Personal Assistance', 'Other'];
...
function normalizeCommandGroup(value) {
  const raw = String(value || '').trim();
  return ALLOWED_COMMAND_GROUPS.includes(raw) ? raw : 'Other';
}
...
function migrateLegacyTriggers() {
  ...
  for (const item of commands) {
    ...
    deduped.push({
      ...
      group: normalizeCommandGroup(item.group || item.category || 'Custom Command'),
...
function createCommand(payload) {
  const { trigger, response, mediaUrl, mediaType, fileName, buttons, group } = payload;
  ...
  const entry = {
    ...
    description: '',
    category: 'General',
    group: normalizeCommandGroup(group || 'Other'),
    createdAt: new Date().toISOString(),
  };
...
function updateCommand(trigger, payload) {
  ...
  const { response, mediaUrl, mediaType, fileName, buttons, group } = payload;
  const cleanResponse = String(response || '').trim();
  const cleanMediaUrl = String(mediaUrl || '').trim();
  const parsedButtons = normalizeButtons(buttons);
  ...
  if (payload && Object.prototype.hasOwnProperty.call(payload, 'group')) {
    target.group = normalizeCommandGroup(group);
  }
